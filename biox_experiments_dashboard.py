...
# ============================================================================
    ...
    if len(exps):
        ...
        runs = q(f"""
            SELECT r.run_id, r.model_type, r.session_tag, ev.evaluation_id, ev.eval_scenario,
...
# ============================================================================
    ...
    scope = q(f"""
        SELECT evaluation_id, project_name, experiment_name, model_type,
               eval_scenario, session_tag, created_at, {metric} AS value
        FROM {EXP_SCHEMA}.v_evaluation_summary
    ...
    if scope.empty:
        ...
        with c2:
            sessions = sorted([s for s in scope["session_tag"].dropna().unique()])
            picked = st.multiselect("Session filter", sessions, default=sessions)
        view = scope if proj == "(all)" else scope[scope["project_name"] == proj]
        if picked:
            view = view[view["session_tag"].isin(picked)]
        view = view.reset_index(drop=True)
        view["seq"] = view.index + 1
        fig = px.line(view, x="seq", y="value", color="experiment_name", markers=True,
                      hover_data=["model_type", "eval_scenario", "session_tag", "evaluation_id"],
        ...
        st.dataframe(view[["evaluation_id", "experiment_name", "model_type",
                           "eval_scenario", "session_tag", "value"]],
                     use_container_width=True, hide_index=True)
...
# ============================================================================
    ...
    runs = q(f"""
        SELECT r.run_id, e.name AS experiment_name, r.model_type, r.session_tag,
               r.mlflow_run_id, r.notes
        FROM {EXP_SCHEMA}.experiment_runs r
    ...
    st.caption(f"MLflow run: `{run_meta['mlflow_run_id']}`  |  session: `{run_meta['session_tag'] or '-'}`  |  notes: {run_meta['notes'] or '-'}")
